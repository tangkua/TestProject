...
<body style="background-color: #617C96">
	...
	<sql:query dataSource="${webappDataSource}"
		sql="select * from tbl_chapter where book_id='${book.getBook_id()}' order by chapter_id asc"
		var="result" />
	...
	<div class="container">
		<div class="row" style="margin-top: 100px; hight: 500px">
			<div class="col-md-5">
				...
				<div class="col-md-12 container book-image" style="width: 100%;">
					<!--image-->
					<img src="ShowBookSevlet?bookid=${book.getBook_id()}"
						alt="ชื่อหนังสือ" class="img-thumbnail center-block"
						style="height: 380px; width: 300px; max-width: 300px; max-hight: 380px;">
				</div>
			...
			<div class="col-md-7">
				...
				<strong class="username">${book.getOwner()} </strong> <strong>
					- </strong> <small class="date-book">${book.getBookdate()} </small><br>

				<div class="tag-issues">
					<span class="glyphicon glyphicon-tag"
						style="font-size: 14px; color: #F49C14"></span> <span
						class="tag-issue">${book.getCategories_name()} </span>
					...
					<c:forEach var="row" items="${result.rows}" varStatus="loop">
						<tr>
							...
							<td class="col-md-3 break-word">
								<h4>
									<a
										href="ShowChapterServlet?chapterid=${row.chapter_id}&userid=${user.getId()}&bookid=${book.getBook_id()}">${row.chapter_name}</a>
								</h4>
								<div style="margin-top: 15px">
									<a href="audio.html">
										<button class="btn btn-info btn-xs">
											<span class="glyphicon glyphicon-play-circle"
												title="จำนวน 3 ออดิโอ">3ออดิโอ</span>
										</button>
									</a>
									
										<form action="DeleteChapterServlet" method="post" style="display:inline!important;"  >
										<input type="hidden" name="chapterid" value="${row.chapter_id}">
											<button type="submit" class="btn btn-info btn-xs" >
												<span class="glyphicon glyphicon-trash" title="Delete"></span>
											</button>
										</form>
										<form style="display:inline!important;" action="SelectChapterServlet" method="post">
										<input type="hidden" name="chapterid" value="${row.chapter_id}"> 
											<button type="submit" class="btn btn-info btn-xs">
							...
							<td class="col-md-7 break-word ">
								...
								<div class="small pull-right" style="margin-top: 30px;">
									...
									&nbsp;<label class="label label-info glyphicon glyphicon-time "
										title="เพิ่มเื่อเวลา....">${row.chapter_datetime}</label>
									&nbsp;<label
										class="label label-info glyphicon glyphicon-heart-empty "
										title="favorite">${row.chapter_countview}</label>
								</div>
							</td>
						</tr>
